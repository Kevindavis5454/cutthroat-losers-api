...
const getPointsSpentBlock = (user_id, contest_id, cb) => {
    pool.query("SELECT user_id, points FROM points WHERE category='block' AND user_id = $1 AND contest_id = $2", [user_id, contest_id], (error, results) => {
        if (error) {
            throw error
        }
        cb(results)
...
const getPointsSpentSabotage = (user_id, contest_id, cb) => {
    pool.query("SELECT user_id, points FROM points WHERE category='sabotage' AND user_id = $1 AND contest_id = $2", [user_id, contest_id], (error, results) => {
        if (error) {
            throw error
        }
        cb(results)
