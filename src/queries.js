...
const adminWeightProgress = (user_id, cb) => {
    pool.query('SELECT CAST(weight AS DOUBLE PRECISION), date_created FROM weighin WHERE user_id = $1 ORDER BY date_created ASC LIMIT 1', [user_id], (error, results) => {
        if (error) {
            throw error
        }
        cb(results)
...
const adminMeasurementProgress = (user_id, cb) => {
    pool.query('SELECT CAST(measurement AS DOUBLE PRECISION), date_created FROM measurements WHERE user_id = $1 ORDER BY date_created ASC LIMIT 1', [user_id], (error, results) => {
        if (error) {
            throw error
        }
        cb(results)
